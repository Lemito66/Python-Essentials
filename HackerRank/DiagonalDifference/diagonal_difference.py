def diagonal_difference(list_bidimentional: list):
    ...
    for i in range(len(list_bidimentional)):
        ...
        lista_diagonal_bottom.append(list_bidimentional[i][len(list_bidimentional)-1-i]) # 0 2 + 1 1 + 2 0
    result = -(sum(lista_diagonal_top)-sum(lista_diagonal_bottom))
    return result
